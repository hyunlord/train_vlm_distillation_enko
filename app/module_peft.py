...
class EnKoDistillationModule(pl.LightningModule):
    ...
    def init_model(self, teacher_model_name: str, student_model_name: str):
        teacher = AutoModel.from_pretrained(teacher_model_name, torch_dtype=torch.int8)
        for param in teacher.parameters():
            param.requires_grad = False
        teacher.eval()

        student = AutoModel.from_pretrained(student_model_name, torch_dtype=torch.int8)
